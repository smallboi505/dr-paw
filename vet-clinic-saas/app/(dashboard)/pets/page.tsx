...
import { Search, Plus, Printer, Upload, Trash2 } from "lucide-react";
...
export default function PetsListPage() {
  ...
  const formatDate = (dateString: string) =>
    new Date(dateString).toLocaleDateString("en-US", { month: "2-digit", day: "2-digit", year: "2-digit" });
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    setSelectedPets(e.target.checked ? pets.map(p => p.id) : []);
  };
  ...
  const handleBulkDelete = async () => {
    ...
    setDeleting(true);
    try {
      await Promise.all(selectedPets.map(id => fetch(`/api/pets/${id}`, { method: "DELETE" })));
  ...
  return (
    <div className="max-w-7xl mx-auto py-4 px-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-slate-600 mt-1">Manage all registered pets</p>
      </div>

      {/* Filters Bar */}
      <div className="bg-white rounded-lg border border-slate-200 px-4 py-3 mb-6">
        <div className="flex items-center gap-3 flex-wrap">
          {/* Tabs - h-9 matches all buttons */}
          <div className="flex rounded-lg border border-slate-200 overflow-hidden flex-shrink-0">
            <button className="h-9 px-5 text-sm font-semibold bg-[#C00000] text-white">
              Pets
            </button>
            <Link href="/owners">
              <button className="h-9 px-5 text-sm font-semibold bg-slate-100 text-slate-600 hover:bg-slate-200 transition-colors">
                Owners
              </button>
            </Link>
          </div>
          ...
          <div className="flex-1 min-w-[180px] relative">
            ...
            <Input placeholder="Search pets..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10 h-9" />
          </div>

          <Select value={speciesFilter} onChange={(e) => setSpeciesFilter(e.target.value)} className="h-9 w-36">
          ...
          <Button variant="outline" size="sm" onClick={() => setShowImportDialog(true)} className="h-9">
          ...
          <Button
            ...
            className={`h-9 ${selectedPets.length > 0 ? "text-red-600 border-red-600 hover:bg-red-50" : "opacity-50 cursor-not-allowed"}`}
          >
          ...
          <Link href="/pets/new">
            <Button size="sm" className="h-9 bg-[#C00000] hover:bg-[#A00000]">
      ...
      <div className="bg-white rounded-lg border border-slate-200 overflow-x-auto">
        <table className="w-full min-w-[800px]">
          <thead className="bg-pink-100 border-b border-slate-200">
            <tr>
              <th className="text-left py-3 px-4 w-12">
                <input type="checkbox" checked={selectedPets.length === pets.length && pets.length > 0} onChange={handleSelectAll} className="w-4 h-4 rounded border-slate-300" />
              </th>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={8} className="text-center py-8 text-slate-500">Loading pets...</td></tr>
            ) : pets.length === 0 ? (
              <tr><td colSpan={8} className="text-center py-8 text-slate-500">No pets found</td></tr>
            ) : (
              pets.map((pet) => (
                <tr key={pet.id} className="border-b border-slate-100 hover:bg-slate-50">
                  <td className="py-3 px-4">
                    <input type="checkbox" checked={selectedPets.includes(pet.id)} onChange={() => handleSelectPet(pet.id)} className="w-4 h-4 rounded border-slate-300" />
                  </td>
                  <td className="py-3 px-4 text-sm">
                    <Link href={`/pets/${pet.id}`} className="text-[#C00000] hover:text-[#A00000] font-semibold hover:underline">{pet.name}</Link>
                  </td>
                  <td className="py-3 px-4 text-sm text-slate-600">{pet.idNumber}</td>
                  <td className="py-3 px-4 text-sm text-slate-600">{pet.species}{pet.breed ? `, ${pet.breed}` : ""}</td>
                  <td className="py-3 px-4 text-sm text-slate-600">{pet.sex}{pet.color ? `, ${pet.color}` : ""}</td>
                  <td className="py-3 px-4 text-sm text-slate-600">{formatDate(pet.createdAt)}</td>
                  ...
                  <td className="py-3 px-4">
                    <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${
                      ...
                      : pet.status === "DECEASED" ? "bg-red-100 text-red-800"
      ...
      <ImportPetsDialog open={showImportDialog} onClose={() => setShowImportDialog(false)} onSuccess={() => { fetchPets(); setShowImportDialog(false); }} />
