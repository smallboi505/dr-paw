...
export default function OwnersListPage() {
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    setSelectedOwners(e.target.checked ? owners.map(o => o.id) : []);
  };
  ...
  const getTotalPetsCount = () =>
    selectedOwners.reduce((total, ownerId) => {
  ...
  const handleBulkDelete = async () => {
    ...
    setDeleting(true);
    try {
      await Promise.all(selectedOwners.map(id => fetch(`/api/owners/${id}`, { method: "DELETE" })));
  ...
  return (
    <div className="max-w-7xl mx-auto py-4 px-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-slate-600 mt-1">Manage all pet owners</p>
      </div>

      {/* Filters Bar */}
      <div className="bg-white rounded-lg border border-slate-200 px-4 py-3 mb-6">
        <div className="flex items-center gap-3 flex-wrap">
          {/* Tabs - same height as buttons via h-9 */}
          ...
          <select
            ...
            onChange={(e) => { setPetCountFilter(e.target.value); setPage(1); }}
            className="h-9 px-3 border border-slate-300 rounded-lg text-sm text-slate-700 bg-white focus:outline-none focus:ring-2 focus:ring-purple-500"
          >
          ...
          <Button
            ...
            className={`h-9 ${selectedOwners.length > 0 ? "text-red-600 border-red-600 hover:bg-red-50" : "opacity-50 cursor-not-allowed"}`}
          >
      ...
      <div className="bg-white rounded-lg border border-slate-200 overflow-x-auto">
        <table className="w-full min-w-[900px]">
          <thead className="bg-pink-100 border-b border-slate-200">
            <tr>
              <th className="text-left py-3 px-4 w-12">
                <input type="checkbox" checked={selectedOwners.length === owners.length && owners.length > 0} onChange={handleSelectAll} className="w-4 h-4 rounded border-slate-300" />
              </th>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={7} className="text-center py-8 text-slate-500">Loading owners...</td></tr>
            ) : owners.length === 0 ? (
              <tr><td colSpan={7} className="text-center py-8 text-slate-500">No owners found</td></tr>
            ) : (
              owners.map((owner) => (
                <tr key={owner.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                  <td className="py-4 px-4">
                    <input type="checkbox" checked={selectedOwners.includes(owner.id)} onChange={() => handleSelectOwner(owner.id)} className="w-4 h-4 rounded border-slate-300" />
                  </td>
                  <td className="py-4 px-4">
                  ...
                  <td className="py-4 px-4">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <a href={`tel:${owner.phone}`} className="text-[#C00000] hover:underline font-medium">{owner.phone}</a>
                    </div>
                  </td>
                  <td className="py-4 px-4">
                  ...
                  <td className="py-4 px-4">
                    <Button variant="ghost" size="sm" onClick={() => setEditingOwner(owner)} className="text-slate-600 hover:text-[#C00000]">
      ...
      {editingOwner && (
        <EditOwnerDialog
          ...
          onSuccess={() => { fetchOwners(); setEditingOwner(null); }}
        />
      )}
